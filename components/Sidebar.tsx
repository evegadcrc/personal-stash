...
export default function Sidebar({ categories, selected, onSelect, unreadCounts }: SidebarProps) {
    ...
    <aside className="w-full flex flex-col gap-1">
      ...
      >
        ...
        <span className="flex items-center gap-1.5">
          {totalUnread > 0 && <span className="h-1.5 w-1.5 rounded-full bg-emerald-500 shrink-0" />}
      ...
      {categories.map((cat) => {
        ...
        return (
          ...
          >
            ...
            <span className="flex items-center gap-1.5">
              {unread > 0 && <span className="h-1.5 w-1.5 rounded-full bg-emerald-500 shrink-0" />}
