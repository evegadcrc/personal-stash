    ...
    <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5">
      {categories.map((cat) => {
        ...
        return (
          <div
            key={cat.name}
            className="group relative flex flex-col rounded-xl border border-zinc-800 bg-zinc-900 p-4 transition-all hover:border-zinc-700 hover:bg-zinc-800/60"
          >
            ...
            >
              ...
              <div className="flex items-center gap-1.5 flex-wrap">
                {unread > 0 ? (
                  ...
                  <span className="text-xs text-zinc-600">{totalCount} {totalCount === 1 ? "item" : "items"}</span>
                )}
                {isShared && <ShareIcon />}
              </div>
            </button>
